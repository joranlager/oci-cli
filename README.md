...
bash
```
docker run -it --rm --mount type=bind,source="$(pwd)",target=/root/.oci --env-file tenancy.env joranlager/oci-cli /bin/bash
```
...
```
docker build -f Dockerfile -t joranlager/oci-cli:3.6.0 --build-arg OCI_CLIENT_VERSION="3.6.0" .
```
